...
function obtenirReqEltsHorsForfaitFicheFraisemploye($unMois) {
    $unMois = filtrerChainePourBD($unMois);
    $requete = "select idVisiteur, date, libelle, montant, Valide from lignefraishorsforfait
              where  mois='" . $unMois . "'";
    return $requete;
	
}
function obtenirnom($unIdVisiteur) {
    $unIdVisiteur = filtrerChainePourBD($unIdVisiteur);
    $requete = "select nom from visiteur where idVisiteur='" . $unIdVisiteur .  "'";
    return $requete;
